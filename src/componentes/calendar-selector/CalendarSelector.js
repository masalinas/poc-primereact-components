...
import { DomHandler, ObjectUtils, classNames, ConnectedOverlayScrollHandler, mask, ZIndexUtils } from 'primereact/utils';
...
export class CalendarSelector extends Component {
    ...
    constructor(props) {
        ...
        // define group intervals enumeration
        this.TODAY = 0;
        this.LAST_2_DAYS = 1;
        this.LAST_7_DAYS = 2;
        this.LAST_30_DAYS = 3;
    ...
    calculateDaysInterval(event, groupDaySelected) {
        let selectedValues = [];
        ...
        switch (groupDaySelected) {
            case this.TODAY:                                
                dateEnd = null;                

                break;
            case this.LAST_2_DAYS:                
                dateStart = new Date(dateStart.setDate(dateEnd.getDate() - 1));       
                
                 break;
            case this.LAST_7_DAYS:
                dateStart = new Date(dateStart.setDate(dateEnd.getDate() - 6));           

                break;
            case this.LAST_30_DAYS:
                dateStart = new Date(dateStart.setDate(dateEnd.getDate() - 29));   
                
                break;
            case this.LAST_60_DAYS:
                dateStart = new Date(dateStart.setDate(dateEnd.getDate() - 59));   

                break;
            case this.LAST_90_DAYS:
                dateStart = new Date(dateStart.setDate(dateEnd.getDate() - 89));   

                break;
            case this.LAST_YEAR:
                dateStart = new Date(dateStart.setDate(dateEnd.getDate() - 364));   
        ...
        selectedValues = [dateStart, dateEnd];
    ...
    onApplyButtonClick(event) {
        ...
        if (this.props.onApplyButtonClick)
            this.props.onApplyButtonClick(event);
    }
    
    renderDaysSelectorPanel() {
        const groupDaysSelections = [
            {label: 'Hoy', value: this.TODAY},
            {label: 'Últimos 2 días', value: this.LAST_2_DAYS},
            {label: 'Últimos 7 días', value: this.LAST_7_DAYS},
            {label: 'Últimos 30 días', value: this.LAST_30_DAYS },
            {label: 'Últimos 60 días', value: this.LAST_60_DAYS },
            {label: 'Últimos 90 días', value: this.LAST_90_DAYS },            
            {label: 'Último año', value: this.LAST_YEAR}
        ];
                
        return (
            <div className="p-dayselector-group-container">
                <ListBox options={groupDaysSelections} onChange={(e) => {                    
                    this.calculateDaysInterval(e, e.value);
                }}/>
                <div className="p-dayselector-group-toolbox">
                    <Button label="Aplicar" onClick={this.onApplyButtonClick}/>
                    <Button label="Resetear" className="p-button-text" onClick={this.onClearButtonClick}/>
                </div>
            </div>
        );
    }
    ...
    renderMonths(monthsMetaData) {
        ...
        const daysSelector = this.renderDaysSelectorPanel();
